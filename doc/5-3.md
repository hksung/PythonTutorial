...
spaCy tokenization creates tokens intended to support linguistic annotation and text analysis.

Language models, however, often divide text into smaller computational units called **subword tokens**.

A subword tokenizer may:

- keep a frequent word as one unit;
- divide a less frequent word into multiple pieces;
- represent punctuation and whitespace using model-specific conventions;
- assign a numerical ID to each subword unit.

Subword tokenization allows a language model to represent unfamiliar words, spelling variants, and morphologically complex forms using a fixed vocabulary.

## A tokenizer belongs to a particular language model

A subword tokenizer is not a general-purpose linguistic tokenizer. It is constructed as part of a particular pretrained language model.

Therefore, the tokenizer used to process a text should normally match the language model that will receive the text.
...
> **Note:** Another valid approach would be to use one multilingual tokenizer, such as multilingual BERT, for both languages. That approach is appropriate when investigating how the same multilingual model represents different languages. Here, however, we use language-specific models to demonstrate that each language model has its own tokenizer and vocabulary.
...
A subword token is not necessarily:

- an orthographic word;
- a spaCy token;
- a lemma;
- a linguistically defined morpheme.

## Comparing subword counts across languages

We can print the number of subwords produced for each sentence:

```python
print(
    "English subwords:",
    len(english_subwords)
)
...
However, these numbers should not be interpreted as a direct comparison of the number of words in the two sentences.

The English and Korean tokenizers have different:
...
A larger number of subwords may reflect the tokenizer's vocabulary rather than greater linguistic complexity.

Because different tokenizers are being used, the resulting token counts are also not a controlled measure of cross-linguistic tokenization efficiency.
...
## Do not use subwords as lemmas

Subword tokenization does not replace linguistic tokenization or lemmatization.

Several inflected forms may receive different subword representations even when they share the same lemma. Conversely, one linguistic word may be divided into several subwords.

For token- or lemma-frequency analysis, use the spaCy tokens or lemmas appropriate for the language.

Use subword tokens when investigating:

- how a particular language model represents text;
- how efficiently its vocabulary covers a language;
- how many model input units a text requires;
- how tokenization may affect language-model processing.
